...
# Ensure printing Unicode (emojis etc.) is safe on Windows consoles.
try:
...
def main():
    ...
    # Configuration
    # Use a raw string or forward slashes to avoid Python treating backslashes as escape sequences
    CHECKPOINT_PATH = r"C:\Users\hp\Downloads\modern_fingat_2025\checkpoints\fingat-epoch=22-val_mrr=0.3333.ckpt"
    DATA_PATH = 'indian_data'
    ...
    try:
        ...
        # --- NEW: write machine-readable top-stocks outputs for automation ---
        try:
            out_dir = Path('data')
            out_dir.mkdir(exist_ok=True)

            # Top-k list (20 by default)
            top_k = results_df.head(20)

            # JSON summary
            json_path = out_dir / f'top_stocks_{datetime.now().strftime("%Y-%m-%d")}.json'
            json_payload = {
                'generated_at': datetime.now().isoformat(),
                'count': len(top_k),
                'top_stocks': top_k.to_dict(orient='records')
            }
            import json as _json
            with json_path.open('w', encoding='utf-8') as jf:
                _json.dump(json_payload, jf, ensure_ascii=False, indent=2)

            # Plain text summary for easy emailing (both dated and latest)
            timestamp = datetime.now()
            ts_short = timestamp.strftime('%Y-%m-%d')
            ts_long = timestamp.strftime('%Y-%m-%d %H:%M:%S')

            # latest (non-dated) text file for quick access
            txt_path = out_dir / 'top_stocks.txt'
            with txt_path.open('w', encoding='utf-8') as tf:
                tf.write(f"Top {len(top_k)} stocks generated: {ts_long}\n\n")
                for i, row in top_k.iterrows():
                    tf.write(f"{i+1}. {row['Ticker']} | Sector: {row.get('Sector','-')} | Dir: {row.get('Direction','-')} | Conf: {row.get('Confidence_%',0):.1f}% | Score: {row.get('Ranking_Score',0):.4f}\n")

            # dated text file for archival / daily automation
            dated_txt_path = out_dir / f'top_stocks_{ts_short}.txt'
            with dated_txt_path.open('w', encoding='utf-8') as dtf:
                dtf.write(f"Top {len(top_k)} stocks generated: {ts_long}\n\n")
                for i, row in top_k.iterrows():
                    dtf.write(f"{i+1}. {row['Ticker']} | Sector: {row.get('Sector','-')} | Dir: {row.get('Direction','-')} | Conf: {row.get('Confidence_%',0):.1f}% | Score: {row.get('Ranking_Score',0):.4f}\n")

            print(f"\n💾 Top-stocks JSON written to: {json_path}")
            print(f"💾 Top-stocks text written to: {txt_path} (latest)")
            print(f"💾 Dated top-stocks text written to: {dated_txt_path}")
        except Exception as _e:
            print(f"\n⚠️ Could not write top-stocks outputs: {_e}")
